...
void	case_c(t_world *world, int x_new, int y_new)
{
	...
	c = world->map->map[y_new / world->image->img_height][x_new
		/ world->image->img_width];
	c2 = world->map->map[world->player->y
		/ world->image->img_height][world->player->x / world->image->img_width];
	if (c2 == 'E')
		mlx_put_image_to_window(world->mlx, world->window, world->image->exit,
			world->player->x, world->player->y);
	else
		mlx_put_image_to_window(world->mlx, world->window, world->image->grass,
			world->player->x, world->player->y);
